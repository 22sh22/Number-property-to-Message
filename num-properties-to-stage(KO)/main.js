...
class PropertySettingModal extends Modal {
    ...
    onOpen() {
        ...
        this.contentEl.createEl('h2', { text: this.property ? '속성 편집' : '새 속성 추가' });
    ...
    createMessageSettings() {
        this.contentEl.createEl('h3', { text: '단계별 메시지' });
        
        this.tempProperty.messages.forEach((message, index) => {
            new Setting(this.contentEl)
                .setName(`${this.tempProperty.range.min + index}단계 메시지`)
                .addText(text => text
                    .setValue(message)
                    .onChange(value => {
    ...
    adjustMessages() {
        ...
        while (this.tempProperty.messages.length < count) {
            ...
            this.tempProperty.messages.push(
                `${this.tempProperty.range.min + index}단계: 설정이 필요합니다`
            );
        }
        this.onOpen();
    }
...
class MetadataCustomizerSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        this.containerEl.createEl('h2', { text: '단계별 설정' });

        new Setting(this.containerEl)
            .setName('새 속성 추가')
            .setDesc('새로운 단계 속성을 추가합니다')
            .addButton(button => button
                .setButtonText('추가')
                .onClick(async () => {
        ...
        this.plugin.settings.properties.forEach((property, index) => {
            new Setting(this.containerEl)
                ...
                .setDesc(`범위: ${property.range.min}-${property.range.max}단계`)
                .addButton(button => button
                    .setButtonText('편집')
                    .onClick(() => {
                        const modal = new PropertySettingModal(
